...
def main():
    ...
    with col2:
        ...
        st.metric(
            label="男性用户",
            value=f"{male_count:,}",
            delta=f"{male_count/len(df)*100:.1f}%"
        )
    ...
    with col3:
        ...
        st.metric(
            ...
            delta=f"{female_count/len(df)*100:.1f}%"
        )
    ...
    with col1:
        st.subheader("📊 性别分布")
        gender_fig = create_gender_chart(df)
        st.pyplot(gender_fig, use_container_width=True)
    
    with col2:
        st.subheader("📈 年龄与性别分布")
        age_gender_fig = create_age_gender_chart(df)
    ...
    with summary_col1:
        ...
        gender_summary['占比'] = (gender_summary['用户数'] / len(df) * 100).round(2).astype(str) + '%'
        st.dataframe(gender_summary, use_container_width=True, hide_index=True)
    
    with summary_col2:
        st.markdown("### 年龄段统计")
        age_summary = df['age_group'].value_counts().reindex(['18-24', '25-34', '35-44', '45-54', '55+']).reset_index()
        age_summary.columns = ['年龄段', '用户数']
        age_summary['占比'] = (age_summary['用户数'] / len(df) * 100).round(2).astype(str) + '%'
